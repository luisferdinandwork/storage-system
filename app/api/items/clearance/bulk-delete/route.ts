// app/api/items/clearance/bulk-delete/route.ts
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ 
        message: `${existingItemIds.length} clearance records deleted successfully`,
        deletedCount: existingItemIds.length,
      ...
      return NextResponse.json({ 
        error: 'Failed to delete clearance records',
        details: error instanceof Error ? error.message : 'Unknown error',
        stockUpdateResults
      }, { status: 500 });
    }
  } catch (error) {
    console.error('Error deleting clearance items:', error);
    return NextResponse.json({ 
      error: 'Failed to delete clearance items',
      details: error instanceof Error ? error.message : 'Unknown error'
    }, { status: 500 });
  }
}